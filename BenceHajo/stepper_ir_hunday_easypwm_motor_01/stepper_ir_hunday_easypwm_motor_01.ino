...
const byte propeller_pin_pwm = 5;
...
void setPropellerSpeed(short pwmIndex) {
  Serial.print("pwmIndex:");
  Serial.println(pwmIndex);

  if (pwmIndex >= MAX_PWM_SETS) {
    //FFWD MAX SPEED
    digitalWrite(propeller_pin_pwm, HIGH);
  ...
  else if (pwm_sets[pwmIndex] == 0) {
    //STOP
    digitalWrite(propeller_pin_pwm, LOW);
    digitalWrite(propeller_pin_direction, LOW);
  }
  else {
    analogWrite(propeller_pin_pwm, pwm_sets[pwmIndex]);
    digitalWrite(propeller_pin_direction, (pwmIndex > 3) ? LOW:HIGH);
  }
}
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    if (results.value == 2578 || results.value == 530) {
      //up
      Serial.println("UP");
      if (propellerVelocity < MAX_PWM_SETS) { 
        propellerVelocity += 1;
